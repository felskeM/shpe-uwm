    ...
    <footer className="mt-16 text-sm border-t border-(--line) footer-bg">
      <Container className="py-10">
        <div className="grid gap-10 md:grid-cols-[1fr_auto] md:items-start">
          {/* Left side: logo, tagline, horizontal nav, socials */}
          <div>
            <Link href="/" className="inline-flex items-center gap-3 group">
              <Image
                ...
                priority
              />
              <p className="leading-tight text-[color-mix(in_oklab,var(--foreground)_70%,transparent)]">
                University of Wisconsin–Milwaukee
              </p>
            </Link>

            <p className="mt-3 text-[color-mix(in_oklab,var(--foreground)_65%,transparent)]">
              Leading Hispanics in STEM at University of Wisconsin–Milwaukee.
            </p>

            {/* NEW: one horizontal nav under the tagline (mobile + desktop) */}
        ...
        {/* Bottom strip */}
        <div className="mt-8 pt-6 border-t border-(--line) text-[color-mix(in_oklab,var(--foreground)_60%,transparent)] flex flex-col sm:flex-row sm:items-center sm:justify-between">
